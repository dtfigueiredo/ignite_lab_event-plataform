...
export function CardLesson(props: LessonProps) {
	...
	return (
		<Link to={`/event/lesson/${props.slug}`} className='flex flex-col group'>
			<span className='text-gray-300 mb-2'>{availableAtDateFormat}</span>

			<div className='p-4 border border-gray-500 rounded group-hover:border-green-700'>
				<header className='flex justify-between items-center mb-4'>
					{isLessonAvailable ? AvailableLesson() : UnavailableLesson()}

					<span className='inline-block px-2 py-[0.125rem] border border-green-300 rounded font-bold text-xs uppercase'>
						{props.type === 'Live' ? 'Ao vivo' : 'PdM'}
					</span>
				</header>
